 ...
 */
fun <T> generateNested(coroutine c: NestedGeneratorController<T>.() -> Continuation<Unit>): Sequence<T> =
        object : NestedIterable<T> {
            override fun nestedIterator(): NestedIterator<T> {
                val iterator = NestedGeneratorController<T>()
                iterator.setNextStep(iterator.c())
                return iterator
            }
        }

class NestedGeneratorController<T> internal constructor() : NestedIterator<T>() {
